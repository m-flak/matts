...
describe('JobPageChanges Implementations', () => {
    ...
    it('Runs the Change Command', (done) => {
        const job: Job = {};
        const command = new JobPageChanges('', async (serviceInstance: JobPageDataService, commandData: ChangeCommandData) => {
            const res = await lastValueFrom(service.changeJobData(commandData as Job));
            return res.status === 200;
        },
        job);

        const res = command.invokeCommand(service)
            .then(rez => {
    ...
    it('Runs commands in a certain order', (done) => {
        ...
        let commands = [
            new JobPageChanges('B', async (serviceInstance: JobPageDataService, commandData: ChangeCommandData) => {
                return false;
            },
            job),
        ...
        Promise.all(commands.map(c => c.invokeCommand(service)))
            .then(actualResults => {
