...
import {
  FailedMonitorWhat,
  InitiatedMonitorWhat,
  MonitorService,
  SuccessfulMonitorWhat,
} from 'src/app/services/monitor.service';
...
export abstract class MonitoredComponent<T> implements OnDestroy {
  ...
  kickoff(extraData?: any) {
    ...
    this.initiatedMonitorWhat = this.monitorService.startMonitor({ id: this.monitorWhatId, data: extraData });

    this.successSub = this.initiatedMonitorWhat.success$
      .pipe(map(r => r as SuccessfulMonitorWhat))
      .subscribe(r => this.onSuccessUpdate(r));

    this.failSub = this.initiatedMonitorWhat.failure$
      .pipe(map(r => r as FailedMonitorWhat))
