...
describe('MonitoredMatButtonDirective', () => {
    ...
    it('should be enabled and have original text after succeeding', fakeAsync(() => {
        spyOn(monitorService, "startMonitor").and.callFake(mw => ({ ...mw, success$: of({...mw, type: MonitorWhatResultType.Success}), failure$: EMPTY }));
    ...
    it('should be enabled with original text and display toast after failing', fakeAsync(() => {
        spyOn(monitorService, "startMonitor").and.callFake(mw => ({ ...mw, success$: EMPTY, failure$: of({...mw, type: MonitorWhatResultType.Failure}) }));
